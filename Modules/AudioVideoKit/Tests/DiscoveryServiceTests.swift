...
struct DiscoveryServiceTests {
    ...
    @Test func streamIsClosedWhenSessionIsDeallocated() async throws {
        let service: OSAllocatedUnfairLock<CaptureDevice.DiscoveryService<MockDiscoverySession>?> =
            OSAllocatedUnfairLock(uncheckedState: makeService())
...
private extension DiscoveryServiceTests {
    ...
    func makeService(
        ...
        deviceTypes: [CaptureDevice.DeviceType] = []
    ) -> CaptureDevice.DiscoveryService<MockDiscoverySession> {
        CaptureDevice.DiscoveryService(
            mediaType: mediaType, deviceTypes: deviceTypes, sessionFactory: { _, _ in mockDiscoverySession })
    }
}
